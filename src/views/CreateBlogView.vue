...
// 主题安装信息（按用户手动操作的四步流程配置）
// tag: 需要 checkout 到特定版本
// branch: 使用 -b 参数指定分支
const themeRepoInfo: Record<string, { repo: string; tag?: string; branch?: string; folder: string }> = {
  // Stack: 使用用户手动测试成功的方式：clone 后 checkout v3.34.1
...
async function installTheme(): Promise<boolean> {
  ...
  try {
    // 步骤 1: git init
    try {
      const gitInit = Command.create('run-git', ['init'], { cwd: projectPath.value })
      const initResult = await gitInit.execute()
      console.log('git init:', initResult.code, initResult.stdout, initResult.stderr)
    } catch (e) {
      console.log('git init 跳过（可能已是 git 仓库）')
    }
    
    installProgress.value = `步骤 2/4: 正在下载 ${selectedTheme.value} 主题...`
    ...
    const gitSubmodule = Command.create('run-git', submoduleArgs, { cwd: projectPath.value })
    ...
    if (output.code !== 0) {
      ...
      if (errorMsg.includes('already exists')) {
        ...
        const gitUpdate = Command.create('run-git', ['submodule', 'update', '--init', '--recursive'], { cwd: projectPath.value })
    ...
    if (themeInfo.tag) {
      ...
      // checkout tag 会进入 detached HEAD 状态，这是正常的
      if (checkoutResult.code !== 0 && !checkoutResult.stderr?.includes('detached HEAD')) {
        console.warn('Checkout 警告:', checkoutResult.stderr)
      }
    }
